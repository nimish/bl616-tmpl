...
// https://occ.t-head.cn/vendor/cpu/index?id=3900588052540035072&key=download#sticky
// //pdf https://github.com/T-head-Semi We use T-head Xuantie E907 as example,
// it have its own instructions!!!
...
TM_INLINE void tm_dot_prod_pack2(mtype_t *sptr, mtype_t *kptr, uint32_t size,
  ...
  __asm__ volatile("mv     t6,%[size]          \n"
                   ...
                   : [sptr] "r"(sptr), [kptra] "r"(kptra), [kptrb] "r"(kptrb),
                     [size] "r"(size)
                   : "t0", "t1", "t2", "t3", "t4", "t5", "t6", "a7", "a4", "a5",
                     "a6");
...
TM_INLINE void tm_dot_prod_gap(mtype_t *sptr, mtype_t *kptr, uint32_t *k_oft,
                               uint32_t size, sumtype_t *result) {
  sumtype_t sum = 0;
  uint32_t i = 0;
