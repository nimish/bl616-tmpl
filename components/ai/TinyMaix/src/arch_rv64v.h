...
// Use this toolchain support rvv inntrinsic:
...
// vtype: Vtype sets width of element in each vector register (e.g., 16-bit,
// 32-bit, …) vl: Vector length CSR sets number of elements active in each
// instruction vstart: Resumption element after trap fcsr(vxrm/vxsat):
...
uint32_t tdot = 0;
TM_INLINE void tm_dot_prod(mtype_t *sptr, mtype_t *kptr, uint32_t size,
                           sumtype_t *result) {
  float sumbuf[PACK_N];
  float sum = 0.f;
  int cnt = size / PACK_N;
...
TM_INLINE void tm_dot_prod_pack2(mtype_t *sptr, mtype_t *kptr, uint32_t size,
                                 sumtype_t *result) {
  float sumbuf[PACK_N];
  mtype_t *kptr0 = kptr;
...
TM_INLINE void tm_dot_prod_gap_3x3x1(mtype_t *sptr, mtype_t *kptr,
                                     uint32_t *k_oft, sumtype_t *result) {
...
TM_INLINE void tm_dot_prod_3x3x1(mtype_t *sptr, mtype_t *kptr,
                                 sumtype_t *result) {
...
//   vint32m4_t vwmacc_vv_i32m4 (vint32m4_t vd, vint16m2_t vs1, vint16m2_t vs2,
//   size_t vl);
uint32_t tdot = 0;
...
TM_INLINE void tm_dot_prod_pack2(mtype_t *sptr, mtype_t *kptr, uint32_t size,
  ...
  if (cnt > 0) {
    ...
    for (int i = 0; i < cnt; i++) {
      ...
      register vint16m2_t p160 asm("v20") =
          vwmul_vv_i16m2(s8, k80, vl); // product i16
      register vint16m2_t p161 asm("v24") = vwmul_vv_i16m2(s8, k81, vl);
      __asm__ volatile("vsetvli zero,zero,e16,m2,d1");
      __asm__ volatile("vwadd.wv %0,%0,%1"
                       : "+vr"(sumv0)
                       : "vr"(p160)); // add sum
      __asm__ volatile("vwadd.wv %0,%0,%1"
                       : "+vr"(sumv1)
...
TM_INLINE void tm_postprocess_sum(int n, sumtype_t *sums, btype_t *bs, int act,
                                  mtype_t *outp, sctype_t *scales,
...
TM_INLINE void tm_postprocess_sum(int n, sumtype_t *sums, btype_t *bs, int act,
                                  mtype_t *outp, sctype_t *scales,
                                  sctype_t out_s_inv, zptype_t out_zp)
#else
TM_INLINE void tm_postprocess_sum(int n, sumtype_t *sums, btype_t *bs, int act,
                                  mtype_t *outp, int32_t *scales, int32_t out_s,
                                  zptype_t out_zp)
#endif
{
#if !TM_FASTSCALE
  if (n < 4) {
...
#else
      sumf =
          sumf > (6 << TM_FASTSCALE_SHIFT) ? (6 << TM_FASTSCALE_SHIFT) : sumf;
#endif
        break;
      default:
        break;
    ...
    vint16m2_t s16 =
        vfncvt_x_f_w_i16m2(sumsf, vl); // rvv0.7 doesn't support rtz
