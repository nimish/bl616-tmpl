...
static inline unsigned long long getCycleCounter() { \
  ...
  do { \
    __asm__ volatile ("rdcycleh %0" : "=r"(cycleh)); \
    __asm__ volatile ("rdcycle %0" : "=r"(cycle)); \
    __asm__ volatile ("rdcycleh %0" : "=r"(cycleh_tmp)); \
...
static int u_task_list(char *write_buffer, int32_t buffer_len, float *usage,
                       float *trap_usage) {
  TaskStatus_t *ts_array;
...
#define tskSUSPENDED_CHAR ('S')
    ...
    for (j = 0; j < ps_tasks_stat.task_num; j++) {
      if (ps_tasks_stat.ts_array[j].xHandle == ts_array[i].xHandle &&
        ...
        n = snprintf(
            write_buffer, buffer_len,
            "%-15.15s\t%c\t%u\t%u\t%u\t%p\t%.4f%%\r\n", ts_array[i].pcTaskName,
            cStatus, (unsigned int)ts_array[i].uxCurrentPriority,
            (unsigned int)ts_array[i].usStackHighWaterMark,
            (unsigned int)ts_array[i].xTaskNumber, ts_array[i].pxStackBase,
            (ts_array[i].ulRunTimeCounter -
             ps_tasks_stat.ts_array[j].ulRunTimeCounter) /
                (float)interval * 100);
        break;
      }
    }
    ...
    if (j == ps_tasks_stat.task_num) {
      n = snprintf(
          write_buffer, buffer_len, "%-15.15s\t%c\t%u\t%u\t%u\t%p\t%.4f%%\r\n",
          ts_array[i].pcTaskName, cStatus,
          (unsigned int)ts_array[i].uxCurrentPriority,
          (unsigned int)ts_array[i].usStackHighWaterMark,
          (unsigned int)ts_array[i].xTaskNumber, ts_array[i].pxStackBase,
  ...
  *trap_usage =
      (trap_run_time - ps_tasks_stat.trap_run_time) / (float)interval * 100;
...
static int u_irq_list(char *write_buffer, int32_t buffer_len) {
  ...
  if (!ps_irqs_stat.irq_array) {
    ...
    // todo: defer free
    ps_irqs_stat.irq_array =
        pvPortMalloc(irq_array_size * sizeof(struct irq_ctx));
  ...
  for (i = 0; i < irq_array_size; i++) {
    ...
    if (irq_count == ps_irqs_stat.irq_array[i].irq_run_time)
      continue;
    n = snprintf(write_buffer, buffer_len,
                 "[%-13.13s]\t%d\t\t\t\t\t\t%.2f%%\r\n", ctx[i]->name,
                 ctx[i]->irqnum,
                 (irq_count - ps_irqs_stat.irq_array[i].irq_run_time) /
                     (float)interval * 100);
...
static void ps_cmd(char *buf, int len, int argc, char **argv) {
  ...
  *buffer = 0;
  ...
  if (ago >= 0) {
    printf(header, (unsigned int)configCPU_CORE_CLOCK_HZ / (1000 * 1000),
           trap_usage, total_usage, (unsigned int)ago);
    puts(buffer);
