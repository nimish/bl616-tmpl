...
void vPortResetRunningTrapCost(void) {
  ullTrapAllCurrentTime +=
      ulSwitchInExtra + ullRunningTrapCost + ulSwitchOutExtra;
...
uint32_t ulPortGetSwitchInExtra(void) { return ulSwitchInExtra; }

/* all trap runtime include prev tasks cost and current task cost. */
uint64_t ullPortGetTrapAllCurrentTime(void) { return ullTrapAllCurrentTime; }
