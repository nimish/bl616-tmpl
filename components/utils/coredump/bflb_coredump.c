...
#define REVERSE(a)                                                             \
  (((a) & 0xff) << 24 | ((a) & 0xff00) << 8 | ((a) & 0xff0000) >> 8 |          \
   ((a) & 0xff000000) >> 24)

#undef read_const_csr
#define read_const_csr(reg)                                                    \
  ({                                                                           \
    register uintptr_t __tmp;                                                  \
    __asm__("csrr %0, " #reg : "=r"(__tmp));                                   \
    __tmp;                                                                     \
  })
...
// uint32_t _$coredump_binary_id$_ __attribute__((used,
// section(".coredump_binary_id"))) = BFLB_COREDUMP_BINARY_ID;
...
/* For stack check */
extern __attribute__((weak)) struct {
  uint32_t addr;
  uint32_t len;
} _dump_sections;
// #define DEBUG
...
uint32_t test_data[] = {0x11111111, 0x22222222, 0x33333333, 0x44444444,
                        ...
                        0xdddddddd, 0xeeeeeeee, 0xffffffff, 0xdeadbeef,
                        0xa5a5a5a5, 0x5a5a5a5a, 0x0000ffff, 0xffff0000};
...
typedef void (*dump_handler_t)(const void *data, ssize_t len,
                               struct crc32_stream_ctx *crc_ctx);

static void dump_ascii(const void *data, ssize_t len,
                       struct crc32_stream_ctx *crc_ctx);
static void dump_base64_byte(const void *data, ssize_t len,
                             struct crc32_stream_ctx *crc_ctx);
static void dump_base64_word(const void *data, ssize_t len,
                             struct crc32_stream_ctx *crc_ctx);
static void dump_wifi_reg_others(const void *data, ssize_t len,
                                 struct crc32_stream_ctx *crc_ctx);
static void dump_csr(const void *data, ssize_t len,
                     struct crc32_stream_ctx *crc_ctx);

static const dump_handler_t dump_handler_list[DUMP_TYPE_MAX] = {
    dump_ascii,           dump_base64_byte, dump_base64_word,
    dump_wifi_reg_others, dump_csr,
};
...
#ifdef DEBUG
    {(uintptr_t)test_data, (unsigned int)sizeof(test_data), DUMP_BASE64_BYTE,
     "test_data_byte"},
    {(uintptr_t)test_data, (unsigned int)sizeof(test_data), DUMP_BASE64_WORD,
     "test_data_word"},
    {(uintptr_t) "asdasdasdasdasdsada", 0, DUMP_ASCII, "test_string"},
#elif defined(BL602)
/*
    {(uintptr_t)0x24B00000, (unsigned int)0x55c, DUMP_BASE64_WORD,
"0x24B00000-0x24B0055C"},
    {(uintptr_t)0x24B08000, (unsigned int)0x560, DUMP_BASE64_WORD,
"0x24B08000-0x24B08560"},
    {(uintptr_t)0x24C00000, (unsigned int)0x3c, DUMP_BASE64_WORD,
"0x24C00000-0x24C0003C"},
    {(uintptr_t)0x24C00800, (unsigned int)0xbc, DUMP_BASE64_WORD,
"0x24C00800-0x24C008BC"},
    ...
    {(uintptr_t)0x28008000, (unsigned int)&__LD_CONFIG_EM_SEL, DUMP_BASE64_WORD,
"EM_REG"}, bouffalo sdk not for bl602 wifi */
#elif defined(BL702)
// {(uintptr_t)0x28008000, (unsigned int)&__LD_CONFIG_EM_SEL, DUMP_BASE64_WORD,
// "EM_REG"},
#elif defined(BL702L)
...
// "EM_REG"},
#elif defined(BL808)
// {(uintptr_t)0x28010000, (unsigned int)&__LD_CONFIG_EM_SEL, DUMP_BASE64_WORD,
// "EM_REG"},
/* #elif defined(WB03)
    {(uintptr_t)0xf0004000, (unsigned int)12, DUMP_CSR, "dump riscv csr"},
    {(uintptr_t)0x28010000, (unsigned int)&__LD_CONFIG_EM_SEL, DUMP_BASE64_WORD,
"EM_REG"}, bouffalo sdk not for wb03 */
#elif defined(BL616)
    {(uintptr_t)0xf0004000, (unsigned int)12, DUMP_CSR, "dump riscv csr"},
// {(uintptr_t)0x28010000, (unsigned int)&__LD_CONFIG_EM_SEL, DUMP_BASE64_WORD,
// "EM_REG"},
...
static void dump_wifi_reg(uint32_t *ptr) {
  ...
  // coex
  write_reg(0x44B00510, (read_reg(0x44B00510) & ~((uint32_t)0x000000FF)) |
                            ((uint32_t)49 << 0));
  write_reg(0x44B00510, (read_reg(0x44B00510) & ~((uint32_t)0x0000FF00)) |
                            ((uint32_t)0 << 8));
  ...
  write_reg(0x44900068, (read_reg(0x44900068) & ~((uint32_t)0x0000FFFF)) |
                            ((uint32_t)(0 | 20 << 0) << 0));
...
static void dump_csr(const void *data, ssize_t len,
  ...
  utils_base64_encode_stream(read_csr_cb, cd_base64_wirte_block, (void *)&ctx);
}
 ...
 * @return result
 */
static void bl_coredump_print(uintptr_t addr, uint32_t len, const char *desc,
                              enum dump_type type) {
  struct crc32_stream_ctx crc_ctx;
  uint32_t crc;
  ...
  dump_handler_list[DUMP_BASE64_WORD]((const void *)&crc,
                                      (ssize_t)sizeof(uint32_t), &crc_ctx);
...
#endif
static void bl_coredump_print_n_k(uintptr_t addr, uint32_t len,
                                  const char *desc, enum dump_type type) {
  uint32_t printed_len;
  uint32_t seg;

  for (seg = 0; seg * BL_COREDUMP_PRINT_SEG_N_K * 1024 < len; seg++) {
    printed_len = seg * BL_COREDUMP_PRINT_SEG_N_K * 1024;
    bl_coredump_print(addr + printed_len,
                      len - printed_len >= BL_COREDUMP_PRINT_SEG_N_K * 1024
                          ? BL_COREDUMP_PRINT_SEG_N_K * 1024
                          : len - printed_len,
                      desc, type);
  }
}
  ...
  case 'd':
    do {
      for (i = 0; i < (sizeof(mem_hdr) / sizeof(mem_hdr[0])); i++) {
        bl_coredump_print_n_k(mem_hdr[i].addr, mem_hdr[i].length,
                              mem_hdr[i].desc, mem_hdr[i].type);
      }
    } while (0);
    return;
...
void bl_coredump_run() {
  ...
  for (cmd_pos = 0; cmd_pos < (sizeof(mem_hdr) / sizeof(mem_hdr[0]));
       cmd_pos++) {
    if (mem_hdr[cmd_pos].length == 0) {
      continue;
    }
    bl_coredump_print_n_k(mem_hdr[cmd_pos].addr, mem_hdr[cmd_pos].length,
                          mem_hdr[cmd_pos].desc, mem_hdr[cmd_pos].type);
  }
  for (cmd_pos = 0; (&_dump_sections + cmd_pos)->addr != 0xffffffff;
       cmd_pos++) {
    if ((&_dump_sections + cmd_pos)->addr == 0)
      break;
    if ((&_dump_sections + cmd_pos)->len == 0)
      continue;
    bl_coredump_print_n_k((&_dump_sections + cmd_pos)->addr,
                          (&_dump_sections + cmd_pos)->len, "predefined",
                          DUMP_BASE64_WORD);
  }
