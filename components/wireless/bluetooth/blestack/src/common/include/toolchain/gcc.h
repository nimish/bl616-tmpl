...
#define ___in_section(a, b, c)                                                 \
  __attribute__((                                                              \
      section("." Z_STRINGIFY(a) "." Z_STRINGIFY(b) "." Z_STRINGIFY(c))))
...
#define __ramfunc                                                              \
  __attribute__((noinline)) __attribute__((long_call, section(".ramfunc")))
#endif /* !CONFIG_XIP */
...
/* These macros allow having ARM __asm__ functions callable from thumb */
...
#if defined(CONFIG_ISA_THUMB2)
/* '.syntax unified' is a gcc-ism used in thumb-2 __asm__ files */
