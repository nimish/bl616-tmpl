...
extern int32_t lpfw_calculate_beacon_delay(uint64_t beacon_timestamp_us,
                                           uint64_t rtc_timestamp_us,
                                           uint32_t mode);
extern int32_t
lpfw_beacon_delay_sliding_win_update(int32_t beacon_delay_us,
                                     uint64_t beacon_timestamp_us);
...
void *platform_malloc(uint32_t size) { return malloc(size); }

void platform_free(void *mem_ptr) { free(mem_ptr); }
...
void platform_post_event(int catalogue, int code, ...) {
  ...
  xReturn =
      xTimerPendFunctionCall(async_event_handler, NULL, code, portMAX_DELAY);
...
void platform_hook_beacon(uint32_t rhd, uint32_t tim, bcn_param_t *param) {
  ...
  /* disable int */
  int irq_rest;
  __asm__ volatile("csrrci %0, mstatus, %1" : "=r"(irq_rest) : "i"(1UL << 3));
  /* get rtc cnt */
  ...
  uint32_t rx_delay_us = (param->beacon_len - 32) * 8 * 2 /
                         bcn_rate; /* delay of (rx_start -> rx_end) */
  uint32_t proce_delay_us =
      (mac_time_now - param->mac_local_tsf_l); /* delay of (rx_end -> now) */
  ...
  beacon_delay_us = lpfw_calculate_beacon_delay(beacon_stamp_us, rtc_stamp_us,
                                                BEACON_STAMP_APP);
