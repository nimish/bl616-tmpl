...
int rtos_task_create(rtos_task_fct func, const char *const name,
                     enum rtos_task_id task_id, const uint16_t stack_depth,
                     void *const params, rtos_prio prio,
...
int rtos_task_init_notification(rtos_task_handle task) { return 0; }
...
void rtos_queue_delete(rtos_queue queue) { vQueueDelete(queue); }
...
void rtos_start_scheduler(void) { vTaskStartScheduler(); }

int rtos_init(void) { return 0; }

rtos_task_handle rtos_get_task_handle() { return xTaskGetCurrentTaskHandle(); }
